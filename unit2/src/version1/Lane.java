...
public class Lane extends Thread implements PinsetterObserver {	
	...
	public void receivePinsetterEvent(PinsetterEvent pe) {
			...
			if (pe.pinsDownOnThisThrow() >=  0) {			// this is a real throw
					...
					else if (pe.pinsDownOnThisThrow() == 10 || pe.getThrowNumber() == 2)		// threw a strike// its not the 10th frame
							canThrowAgain = false;
							//publish( lanePublish() ); 
...
//		getScore( Cur, frame );
		cumulScores = new ComputeScore(Cur,frame, scores,bowlIndex,cumulScores,ball).getCumScore(); 
		publish( lanePublish() );
	}
